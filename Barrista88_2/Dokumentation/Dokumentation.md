...
![Bild der Kaffeemaschine](./Bilder/Kaffee.png)
...
![Beispielbield DIV](./Bilder/Display.png)
_Abbildung: DIVs positionieren und anpassen_

![Beispielbild DIVs angepasst](./Bilder/Display_korrekt.png)
...
![Beispielbild Maschine An](./Bilder/AnAus.png)
...
cursor: pointer;
```
![Bild des Mauscursors](./Bilder/Cursor.png)
...
![Bild Maschine Aus](./Bilder/MaschineAus.png)
_Abbildung: Maschine ist aus_
![Bild des Hauptmenüs](./Bilder/Hauptmenü.png)
...
![Bild Telefondisplay](./Bilder/IMG_4588.png)
...
![Bild ELemente stehen über](./Bilder/Buttons.png)
...
![Bild von dem neuen, geordnetem Menü](./Bilder/bfunktionierendes%20Menü.png)
_Abbildung:Das Menü ist jeztzt neu angeordnet un steht nicht über.

![Bild kleines Display im Querformat](./Bilder/Kleines%20Display.png)
...
```JavaScript
  function betriebsstoffePrüfen(auswahl){
                    switch(auswahl){
                        case "Kaffee":
                            if(bohnen >=10 && wasser >=0.3) return true;
                            else return false;
                            break;
                        case "Brühe":
                            if(brühe >= 10 && wasser >= 0.3) return true;
                            else return false;
                            break;
                        case"Kakao":
                            if(milch >= 0.3 && kakao >= 10) return true;
                            else return false;
                        case"Milch":
                            if(milch >= 0.3)return true;
                            else return false;
                    }
                }
``` 
...
![Bild des Geheimmenüs](./Bilder/Geheimmenü.png)
...
![Abbildung Funktionsfehler](./Bilder/laufende%20funktionpng.png)
...
![Bild des Auffüllen-Meüs](./Bilder/Auffüllen.png)
...
![Bild UNtermenü mit Zurück-Knopf](./Bilder/Untermenü.png)
...
Da es nur dieses eine Untermenü gibt, setzt der Klick die Variable auf das Obermenü und ruft die zeigeMenü Funktion auf, welche dann die Statistikfunktion des Auffüllen-Menüs neu aufruft, wodurch die Stände dann aktualisiert werden.

Da von den flüssigen Zutaten je Durchgang 0,3 abgezogen wurden, hat sich folgender Fehler bei der Ausgabe eingeschlichen: 

![Bild vom Fehler der Füllstandsausgabe](./Bilder/Mengenangabe.png)

_Abbildung: Fehler in der Ausgabe_

Bei 1.4 - 0.3 ist das Ergebnis 1.0999999999999999 statt 1.1.

Ich habe die Funktion der Füllstandsanzeige so umgeschriben, dass die Stände von Wasser und Milch auf genau 1 Nachkommastelle präzisiert werden.
...
```JavaScript
                ...
                function füllstandskontrolle(){
                    const stoffe = ["Kaffeebohnen", "Brühe", "Kakaopulver", "Wasser(l)", "Milch(l)"];
                    const stände = [bohnen, brühe, kakao, wasser.toPrecision(2), milch.toPrecision(2)];
...
Da das Menü nun auch während eines Zubereitungsvoorganges aufrufbar war, habe ich einen Check geschrieben, der dies verhindert.
...
angelegt. 
Sobald die Fortschrittsanzeigefunktion aufgerufen wird, setzt diese den Wert auf **true** und nach Abschluss wieder zurück auf **false**.
Die Funktion des Event Listeners für den Geheimmenü-button prüft nun sowohl den As/Aus Zustand wie auch die aktive Zubereitung über die entsprechenden Variablen.
...
Das Menü kann nun nicht aufgerufen werden, wenn die Maschine aus ist (!power) aber auch nicht, falls ein Getränk in Zubereitung ist (zubereitunAktiv = true). 

Ich habe mir von der KI ein Bild einer Kaffetasse erstellen lassen.
Auch dafür habe ich in meinem HTML-Code ein "div" erstellt in welchem das Bild eingebettet ist und dieses über die Methode mit dem roten Rahmen an die entsprechende Position gebracht un die Größenverhältnisse angepasst.
Das Bild hatte einen weißen Hintergrund, ich wollte aber einen transparenten haben, um nur die Tasse anzuzeigen.
Nach einigem Ausprobieren und recherchieren habe ich das Bild auf mein iPhone transferiert. Dort ließ sich das Objekt sehr einfach aus dem Bild lösen und als PNG speichern, welches ich dann entsprechend meiner Vrostellungen wverwenden kpnnte.

Die Fortschrittsfunktion für Getränke habe ich so erweitert, dass dieses Element beim Aufruf der Funktion nun eingeblendet wird, um die Illusion abzurunden. Am Ende ist ein Klick auf die Tasse erforderlich, um diese zu "entnehmen" und zum Hauptmenü zurückzukehren.

```JavaScript
const kaffeeTasse = document.getElementById("tasse");
...
Für den letzten Feinschliff habe ich meinen CSS code an diversen Stellen angepasst, um Elemente entsprechend mener Vorstellungen besser zu positionieren und zu zentrieren und die Größen anzupassen. 

Dadurch, dass die die Fortschrittsanzeigen der Reinigung und der Getränkezubereitung auf das selbe HTML Element zu Anzeigezwecken zugegriffen haben, kam es zu Darstellungsfehleren/Überlagerungen, wenn man eines durch Betätigen des An-Aus-Schalters abgebrochen und dann das Andere aufgerufen hat. 
Ich habe daher ein zusätzliches HTML Element erstellt und die die Funktionen in JS jeweils so angepasst, dass sich ejde Funktion eines anderen Elementes bedient.
...
Ich kam daurch zeitlich etwas in den Verzug, sodass ich  einige Funktionen aus dem Vorgangsprojekt, welche für mich nebensächlich waren, ausgesetzt wurden.
